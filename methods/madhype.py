...
def directional_matches(a_wells,b_wells,a_uniqs,b_uniqs,w_tot,threshold=0.99,silent=False,distinct=None):
    ...
    for ind,au in enumerate(a_uniqs): # enumerate just to get a sense of completion
        ...
        print '\nFinished {}/{}'.format(ind,len(a_uniqs)) 
...
def solve(data,pair_threshold = 0.99,verbose=0,real_data=False):
    ...
    if real_data:
        ...
        # graph reduction
        #results_ab = reduce_graph(all_edges,all_freqs,all_scores,all_uniques)
        #results_aa = reduce_graph(all_edges,all_freqs,all_scores,all_uniques)
        #results_bb = reduce_graph(all_edges,all_freqs,all_scores,all_uniques)
